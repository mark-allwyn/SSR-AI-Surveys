...
**Report Generated:** 2025-10-22 16:50:07
...
- **Human Response Accuracy:** 93.7%
- **LLM Response Accuracy:** 93.0%
- **Accuracy Gap:** 0.7% (Human superior)

### What This Means

Both human and LLM-style responses achieve **comparable** accuracy (0.7% difference). The SSR method effectively handles both response styles.
...
|--------|-------|-----|--------|----------------|
| **Mode Accuracy** | 93.7% | 93.0% | Human | % of predictions exactly matching ground truth |
| **Top-2 Accuracy** | 100.0% | 100.0% | LLM | % where true answer is in top 2 predictions |
| **Mean Absolute Error** | 0.063 | 0.073 | Human | Average distance from true rating (lower is better) |
| **Prob at Truth** | 0.620 | 0.564 | Human | Avg probability assigned to true answer (higher = more confident) |
...
- **Result:** Human responses achieve 93.7% vs LLM's 93.0%
- **Implication:** Direct language is 0.7% more likely to be correctly classified
...
- **Result:** Human MAE = 0.063, LLM MAE = 0.073
...
- **Result:** Human 0.620 vs LLM 0.564
---
...
| Metric | Human | LLM | Difference |
|--------|-------|-----|------------|
| Mode Accuracy | 62.0% | 84.0% | -22.0% |
| MAE | 0.380 | 0.160 | +0.220 |
| Prob at Truth | 0.428 | 0.406 | +0.022 |

#### Interpretation

**5-point Likert scales** are more challenging because SSR must differentiate between 5 similar options. Human responses achieve moderate accuracy (62.0%), which is reasonable given 5 options.

#### Prediction Patterns

- **Human correct predictions:** 31 / 50 responses
...
| Prob at Truth | 0.280 | 0.249 | +0.031 |
...
| Prob at Truth | 0.424 | 0.400 | +0.024 |
...
| Mode Accuracy | 100.0% | 100.0% | +0.0% |
| MAE | 0.000 | 0.000 | +0.000 |
| Prob at Truth | 0.587 | 0.512 | +0.075 |
...
Direct, opinionated responses (human-style) outperform hedged, nuanced responses (LLM-style) by **0.7%** on average. This suggests:
...
**Likert scales** show more variation (87.3% accuracy). The challenge increases with more scale points due to semantic overlap between adjacent options.
...
The average probability assigned to the true answer is 62.0%, meaning SSR typically distributes probability across 3-4 plausible ratings rather than being overly confident. This probabilistic approach:
